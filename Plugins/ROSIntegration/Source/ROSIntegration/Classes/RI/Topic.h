...
protected:
	...
	UFUNCTION(BlueprintImplementableEvent, Category = ROS)
	void OnOdometryMessage(const FVector& position, const FVector4& orientation, const FVector& linear, const FVector& angular);

	UFUNCTION(BlueprintImplementableEvent, Category = ROS)
	void OnOccupancyGridMessage(const TArray<int32>& data, const int32& width, const int32& height, const float& resolution, const FVector& origin);

	UFUNCTION(BlueprintImplementableEvent, Category = ROS)
	void OnPathMessage(const TArray<FVector>& path);

	UFUNCTION(BlueprintImplementableEvent, Category = ROS)
	void OnImageMessage(const UTexture2D* Texture, const int32& width, const int32& height);
