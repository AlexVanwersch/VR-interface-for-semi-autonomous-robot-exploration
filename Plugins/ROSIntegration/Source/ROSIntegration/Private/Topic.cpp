...
, _Implementation(new UTopic::Impl())
{
	...
	if (SupportedMessageTypes.Num() == 0)
	{
		...
		SupportedMessageTypes.Add(EMessageType::OccupancyGrid,	TEXT("nav_msgs/OccupancyGrid"));
		SupportedMessageTypes.Add(EMessageType::Path,			TEXT("nav_msgs/Path"));
...
bool UTopic::Subscribe()
{
	...
	if (_State.Connected)
	{
		...
		std::function<void(TSharedPtr<FROSBaseMsg>)> Callback = [this, MessageType](TSharedPtr<FROSBaseMsg> msg) -> void
		{
			...
			{
				auto ConcreteVectorMessage = StaticCastSharedPtr<ROSMessages::nav_msgs::Odometry>(msg);
				if (ConcreteVectorMessage.IsValid())
				{
					const auto p = ConcreteVectorMessage->pose.pose.position;
					const auto q = ConcreteVectorMessage->pose.pose.orientation;
					const auto r = ConcreteVectorMessage->twist.twist.linear;
					const auto s = ConcreteVectorMessage->twist.twist.angular;
					const FVector position(p.x, p.y, p.z);
					const FVector4 orientation(q.x, q.y, q.z, q.w);
					const FVector linear(r.x, r.y, r.z);
					const FVector angular(s.x, s.y, s.z);
					TWeakPtr<UTopic, ESPMode::ThreadSafe> SelfPtr(_SelfPtr);
					AsyncTask(ENamedThreads::GameThread, [this, position, orientation, linear, angular, SelfPtr]()
						{
							if (!SelfPtr.IsValid()) return;
							OnOdometryMessage(position, orientation, linear, angular);
						});
				}
				break;
			}
			case EMessageType::OccupancyGrid:
			{
				auto ConcreteVectorMessage = StaticCastSharedPtr<ROSMessages::nav_msgs::OccupancyGrid>(msg);
				if (ConcreteVectorMessage.IsValid())
				{
					const auto Data = ConcreteVectorMessage->data;
					const auto Width = ConcreteVectorMessage->info.width;
					const auto Height = ConcreteVectorMessage->info.height;
					const auto Resolution = ConcreteVectorMessage->info.resolution;
					const auto origin = ConcreteVectorMessage->info.origin.position;
					const FVector Origin(origin.x, origin.y, origin.z);

					TWeakPtr<UTopic, ESPMode::ThreadSafe> SelfPtr(_SelfPtr);
					AsyncTask(ENamedThreads::GameThread, [this, Data, Width, Height, Resolution, Origin, SelfPtr]()
						{
							if (!SelfPtr.IsValid()) return;
							OnOccupancyGridMessage(Data, Width, Height, Resolution, Origin);
						});
				}
				break;
			}
			...
			{
				auto ConcreteVectorMessage = StaticCastSharedPtr<ROSMessages::nav_msgs::Path>(msg);
				if (ConcreteVectorMessage.IsValid())
				{
					// Extract the number of poses in the path
					int numPoses = ConcreteVectorMessage->poses.Num();

					// Initialize a TArray to hold the path poses
					TArray<FVector> Path;
					Path.SetNumZeroed(numPoses);
					
					// Copy the pose positions from the Path messages into the TArray
					for (int i = 0; i < numPoses; i++)
					{
						ROSMessages::geometry_msgs::PoseStamped pose = ConcreteVectorMessage->poses[i];
						Path[i] = FVector(pose.pose.position.x, pose.pose.position.y, pose.pose.position.z);
					}
					TWeakPtr<UTopic, ESPMode::ThreadSafe> SelfPtr(_SelfPtr);
					AsyncTask(ENamedThreads::GameThread, [this, Path, SelfPtr]()
						{
							if (!SelfPtr.IsValid()) return;
							OnPathMessage(Path);
						});
			...
			{
				auto ConcreteVectorMessage = StaticCastSharedPtr<ROSMessages::sensor_msgs::Image>(msg);
				if (ConcreteVectorMessage.IsValid())
				{
					const int32 Width = ConcreteVectorMessage->width;
					const int32 Height = ConcreteVectorMessage->height;
					const uint8_t* Data = ConcreteVectorMessage->data;
					...
					memcpy(TextureData, Data, Width* Height * 4);
					...
					TWeakPtr<UTopic, ESPMode::ThreadSafe> SelfPtr(_SelfPtr);
					AsyncTask(ENamedThreads::GameThread, [this, Texture, Width, Height, SelfPtr]()
						{
							if (!SelfPtr.IsValid()) return;
							OnImageMessage(Texture, Width, Height);
						});
				}
				break;
			}
